...
<script>
    ...
    const searchOptions = ['商品id', '价格','商品名', '库存','销量','图片地址','详情','修改时间','创建时间'];
    export default {
        data() {
            return {
                rawList: [],
        ...
        methods: {
            ...
            initTable(){
                axios.get("/api/product/selectAll/"+window.sessionStorage.getItem("id")).then(response => {
            ...
            doAdd: function (){
                this.addPer.busmanId=window.sessionStorage.getItem("id");
