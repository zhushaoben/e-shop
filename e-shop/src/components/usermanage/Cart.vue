<template>
    <el-table
        ...
        <el-table-column label="数量" width="200" v-slot="scope">
            <el-input-number @change="handleChange(scope.$index, scope.row)" size="mini" :min="1" :max="scope.row.stock" v-model="scope.row.number" style="margin-left: 10px"></el-input-number>
        </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            init(){
                axios.get("/api/cart/select/"+window.sessionStorage.getItem("id")).then(response =>{
